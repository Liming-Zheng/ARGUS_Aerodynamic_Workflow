...
def interp_control(eta: float, control_etas: list[float], values: list[float]) -> float:
    if eta <= control_etas[0] or eta >= control_etas[-1]:
        return 0.0
...
def main():
    ...
    baseline = project / "outputs" / "refined_baseline" / "baseline_wing_only_refined.vsp3"
    ...
    for case in cases:
        ...
        schedule = []
        for row in rows:
            increment = interp_control(row["eta"], control_etas, control_twist)
