...
def aerodynamic_coefficients(vsp, result_id: str) -> dict[str, float]:
    """Return explicitly named near- and far-field aerodynamic quantities."""
    cdi_near = _last(vsp, result_id, ["CDi"])
    cdi_far = _last(vsp, result_id, ["CDiw"])
    return {
        "CL": _last(vsp, result_id, ["CLtot", "CL"]),
        "CD": _last(vsp, result_id, ["CDtot", "CD"]),
        # Retain CDi as a compatibility alias; new optimization must use CDiw.
        "CDi": cdi_near,
        "CDi_near_field": cdi_near,
        "CDiw_far_field": cdi_far,
        "E_near_field": _last(vsp, result_id, ["E"]),
        "Ew_far_field": _last(vsp, result_id, ["Ew"]),
...
def evaluate(
    ...
    *,
    ...
    result = {
        ...
        **aerodynamic_coefficients(vsp, history),
    }
    if not loads:
        return result, []
...
def sweep_points(
    ...
    return [
        {
            "alpha_deg": alpha,
            **aerodynamic_coefficients(vsp, result_id),
        }
