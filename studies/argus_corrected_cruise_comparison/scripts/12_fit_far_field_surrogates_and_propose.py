"""Fit CDiw surrogates and propose a far-field exact-validation batch."""
...
def collect_training_rows(project: Path, batch: int) -> list[dict]:
    """Load audited exact results and recover each geometry's commands."""
    del batch  # all audited far-field batches are valid training evidence
    audit_root = project / "outputs" / "far_field_audit"
    rows = read_rows(audit_root / "far_field_ranked_results.csv")
    enriched: list[dict] = []
    for row in rows:
        if row["concept"] not in {"trailing_edge", "twist"}:
            continue
        design_path = audit_root / "cases" / row["case_id"] / "source_design.json"
        design = json.loads(design_path.read_text(encoding="utf-8"))
        commands = design["control_values"]
        enriched.append(
            {
                **row,
                **{name: value for name, value in zip(CONTROL_COLUMNS, commands)},
            }
        )
    return enriched
...
def fit_models(
    ...
    targets = {
        "CDiw_far_field": np.array(
            [float(row["CDiw_far_field"]) for row in rows]
        ),
...
def main() -> None:
    ...
    audited_rows = read_rows(
        project / "outputs" / "far_field_audit" / "far_field_ranked_results.csv"
    )
    baseline_rows = {
        row["flight_state"]: row
        for row in audited_rows
        if row["case_id"] == "rigid_baseline"
    }
    root_limit = float(
        json.loads(
            (
    ...
    output = (
        project
        / "outputs"
        / "far_field_optimization"
        / f"surrogate_batch_{args.batch:02d}"
    )
    ...
    for concept in concepts:
        ...
        for state_index, state in enumerate(["early_cruise", "late_cruise"]):
            ...
            cdi_mean, cdi_std = models["CDiw_far_field"].predict(
        ...
        chosen = first_distinct(
            [
                ("early_min_cdiw", early_rank),
                ("early_constraint_boundary", boundary_rank),
                ("late_min_cdiw", late_rank),
        ...
        for candidate_number, (reason, index) in enumerate(chosen, start=1):
            ...
            case_id = (
                f"ff_{prefix}_b{args.batch:02d}_c{candidate_number:02d}_"
                f"{reason}"
            )
            ...
            design = {
                ...
                "seed_type": f"far_field_surrogate_batch_{args.batch:02d}",
            ...
            for state in ["early_cruise", "late_cruise"]:
                ...
                prediction_row.update(
                    {
                        f"{state}_predicted_CDiw": prediction["cdi_mean"][
                            index
                        ],
                        f"{state}_CDiw_std": prediction["cdi_std"][index],
