...
def main() -> None:
    ...
    output = args.output_root or (
        project / "outputs" / "corrected_low_speed" / "cases" / f"batch_{args.batch:02d}"
    )
    ...
    for case in cases:
        ...
        vsp.ClearVSPModel()
        vsp.ReadVSPFile(str(args.baseline_model.resolve()))
        ...
        section_rows = apply_segmented_hinged_ailerons(
            vsp,
            wing_id,
            case.get("segments", settings["segments"]),
            case["segment_deflections_deg"],
            float(case.get("hinge_x_over_c", settings["hinge_x_over_c"])),
        )
        vsp.Update()
        ...
        design = {
            **case,
            ...
            "hinge_x_over_c": case.get(
                "hinge_x_over_c", settings["hinge_x_over_c"]
            ),
            "segments": case.get("segments", settings["segments"]),
        }
