...
def parse_args():
    ...
    parser.add_argument(
        "--optimization-root",
        type=Path,
        help="Independent sample/result root. Default: outputs/optimization.",
    )
    parser.add_argument(
        "--optimization-config",
        type=Path,
        help="Solver constraint config. Default: config/optimization_config.json.",
    )
...
def worker(args):
    ...
    if result_path.exists() and not args.force:
        return
    config = json.loads((project / "config" / "baseline_config.json").read_text())
    ...
    maximum_delta = float(opt_config["max_adjacent_delta"])
    row = {
        "case_id": case_id,
        **{f"A{i+1}_over_c": value for i, value in enumerate(design["control_amplitudes_over_c"])},
        ...
        "max_adjacent_delta": design["max_adjacent_delta"],
        ...
        "feasible": (
            ...
            and float(design["max_adjacent_delta"]) <= maximum_delta
        ),
    }
