...
def candidate_designs(values_batch, config, iteration):
    ...
    for batch_index, values in enumerate(values_batch, start=1):
        ...
        designs.append({
            ...
            "x_h_over_c": variables["x_h_over_c"],
            ...
            "alpha_deg": 2.0,
            "mach": 0.10,
            "notes": f"User optimization iteration {iteration}",
        })
    return designs
...
def main():
    ...
    for iteration in range(completed_iterations + 1, maximum_iterations + 1):
        ...
        run_command(
            [
                openvsp_python,
                project / "scripts" / "04_generate_representative_cases.py",
                "--cases",
                cases_path,
                "--output-root",
        ...
        run_command(
            [
                openvsp_python,
                project / "scripts" / "15_run_optimization_samples.py",
                "--workers",
